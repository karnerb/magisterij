...
for file in files:
    ...
    data = data[data['beta'].between(1/1.128, 1/1.1235)]
    dataset.append(data)
...
print(m,b)
plt.plot(np.log(avgs[0]['T']-1.1235), m*np.log(avgs[0]['T']-1.1235)+b, label = r'$\alpha = {0:.2f}$'.format(m), c='black')
plt.xlabel('$\log(T-T_{NI})$')
plt.ylabel('$\log(C_V)$')
plt.title(r'$C_V\propto (T-T_{NI})^{-\alpha}$')
plt.legend()
plt.show()
